...
def main():
    # Use the script directory for input/output
    data_dir = Path(__file__).parent
    # If aggregated 15-min files already exist, skip aggregation
    existing_15min = list(data_dir.glob('*15min.csv'))
    if existing_15min:
        print(f"Found {len(existing_15min)} 15-min aggregated file(s) in {data_dir}, skipping aggregation.")
        return
    # Optionally, a specific combined file can be named 'weather_energy.csv'
