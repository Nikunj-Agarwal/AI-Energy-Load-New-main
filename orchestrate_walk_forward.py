...
SUMMARY_CSV = RUNNING_EVAL_DIR / "walk_forward_per_split_summary_predicted.csv"

PREDICTED_OUTPUT_PATH = RESULTS_DIR / "predicted_gkg_energy.csv"
PREDICTION_PLOT_PATH = RESULTS_DIR / "prediction_vs_actual_gkg_energy.png"

def main():
    print("=== Walk-Forward Evaluation with Optuna ===")

    data_path = OUTPUT_DIR / "merged_data" / "gkg_energy.csv"
    ...
    while train_end + test_len <= total_len:
        ...
        # Reset index to make timestamp a column before saving
        temp = test_df[[TARGET_COL]].copy()
        temp["Predicted"] = y_pred
        temp = temp.reset_index()  # Timestamp index -> column (usually named 'index')
    ...
    combined_pred_df.to_csv(PREDICTED_OUTPUT_PATH, index=False)  # Save with timestamp as a column
    print(f"✅ Predictions saved: {PREDICTED_OUTPUT_PATH}")

    plt.figure(figsize=(14, 6))
    # Use the timestamp column (first column after reset_index)
    timestamp_col = combined_pred_df.columns[0]
    plt.plot(combined_pred_df[timestamp_col], combined_pred_df[TARGET_COL], label="Actual", alpha=0.7)
    plt.plot(combined_pred_df[timestamp_col], combined_pred_df["Predicted"], label="Predicted", alpha=0.7)
    plt.title("Actual vs Predicted Power Demand (GKG + Energy)")
